...
if ($status === 'PAID' || $status === 'SETTLED') {
    ...
    if (!$stmt->execute()) {
        ...
        try {
            $adminNotif = $conn->prepare("INSERT INTO admin_notifications (order_id, type, title, message, created_at) VALUES (?, 'payment_received', 'Pembayaran Diterima', ?, NOW())");
            $adminMsg = "Pembayaran untuk pesanan #$ext telah diterima via Xendit.";
            $adminNotif->bind_param("is", $orderId, $adminMsg);
            $adminNotif->execute();
        } catch (Exception $e) {
            error_log("XENDIT WEBHOOK: Failed to notify admin - " . $e->getMessage());
        }

        // Notify User of Payment (if user_id exists)
        if (!empty($order['user_id'])) {
             try {
                $userNotif = $conn->prepare("
                    INSERT INTO notifications 
                    (user_id, order_id, title, message, type, icon, action_url, data, is_read, created_at) 
                    VALUES (?, ?, 'Pembayaran Berhasil', ?, 'payment_received', 'credit-card-2-front', ?, '{}', 0, NOW())
                ");
                $userMsg = "Pembayaran untuk pesanan #$ext berhasil. Pesanan Anda sedang diproses.";
                $actionUrl = "/orders/$ext";
                $uId = (int)$order['user_id']; // Variable for bind_param reference
                $userNotif->bind_param("iiss", $uId, $orderId, $userMsg, $actionUrl);
                $userNotif->execute();
            } catch (Exception $e) {
                error_log("XENDIT WEBHOOK: Failed to notify user - " . $e->getMessage());
            }
        }
    }
